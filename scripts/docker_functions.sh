...
function ssh_start_hnn_print {
  ...
  # ********** For testing ***************
  COMMAND=(6 "$docker_cmd" "exec" "$HNN_CONTAINER_NAME" \
          "bash" "-c" "ls -ld \"\$SYSTEM_USER_DIR/hnn_out\" && ls -l \"\$SYSTEM_USER_DIR/hnn_out\"")
  convert_COMMAND_to_escaped_array
...
function start_hnn_print {
  ...
  COMMAND=(12 "$docker_cmd" "exec" "--env" "TRAVIS_TESTING=$TRAVIS_TESTING" \
          "--env" "DISPLAY=$__display" "-u" "$__container_user" "$HNN_CONTAINER_NAME" \
          "bash" "--login" "/home/hnn_user/start_hnn.sh")
...
function fix_hnn_out_perms_host_fail {
  ...
  find "${SYSTEM_USER_DIR}/hnn_out" -type d -exec chmod 777 {} \;  >> "$LOGFILE" 2>&1 && \
    find "${SYSTEM_USER_DIR}/hnn_out" -type f -exec chmod 777 {} \; >> "$LOGFILE" 2>&1
  fail_on_bad_exit $?
...
function fix_hnn_out_perms_container_print {
  ...
  if [[ "$CONTAINER_TYPE" == "linux" ]]; then
    ...
    __container_user="ContainerAdministrator"
  fi
  ...
  # ************** for debugging ***************
  COMMAND=(6 "$docker_cmd" "exec" "$HNN_CONTAINER_NAME" "bash" "-c" "ls -ld \"${SYSTEM_USER_DIR}/hnn_out\" && ls -l \"${SYSTEM_USER_DIR}/hnn_out/\"")
  convert_COMMAND_to_escaped_array
  MSYS_NO_PATHCONV=1 output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
  # ********************************************
  ...
  if [[ $? -ne 0 ]]; then
    ...
    COMMAND=(8 "$docker_cmd" "exec" "-u" "$__container_user" "$HNN_CONTAINER_NAME" "bash" "-c" \
            "find \"${SYSTEM_USER_DIR}/hnn_out\" -type d -exec chmod 777 {} \; && \
             find \"${SYSTEM_USER_DIR}/hnn_out\" -type f -exec chmod 777 {} \; || true)")
    convert_COMMAND_to_escaped_array
    MSYS_NO_PATHCONV=1 output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
  else
    echo "done" | tee -a "$LOGFILE"
...
function copy_hnn_source_fail {
  ...
  COMMAND=(4 "$docker_cmd" "cp" "$PWD" "$HNN_CONTAINER_NAME:$__source_code_path")
  convert_COMMAND_to_escaped_array
  output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
  fail_on_bad_exit $?
  if [[ "$CONTAINER_TYPE" == "windows" ]]; then
...
function copy_hnn_out_fail {
  ...
  local __hnn_out_path
  __hnn_out_path="$SYSTEM_USER_DIR/hnn_out"

  if [[ ! -e "$__hnn_out_path" ]]; then
    echo -e "\nError: hnn_out doesn't exist at $__hnn_out_path" | tee -a "$LOGFILE"
    cleanup 2
  fi
  ...
  if [[ "$OS" == "mac" ]] && [[ $TRAVIS_TESTING -eq 1 ]]; then
    ...
    COMMAND=(8 "$docker_cmd" "exec" "-u" "root" "$HNN_CONTAINER_NAME" "bash" "-c" \
             "mkdir -p \"$__hnn_out_path/data\" && \
              mkdir -p \"$__hnn_out_path/param\" && \
              chown -R hnn_user:hnn_group \"$__hnn_out_path\" && \
              chmod 777 -R \"$__hnn_out_path\"")
    convert_COMMAND_to_escaped_array
    output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
    fail_on_bad_exit $?
  else
    print_header_message "Removing old hnn_out from container... "
    COMMAND=(6 "$docker_cmd" "exec" "$HNN_CONTAINER_NAME" "bash" "-c" "rm -rf $__hnn_out_path/*")
    convert_COMMAND_to_escaped_array
    output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
    fail_on_bad_exit $?
  fi
  ...
  COMMAND=(4 "$docker_cmd" "cp" "$__hnn_out_path/." "$HNN_CONTAINER_NAME:$__hnn_out_path/")
  convert_COMMAND_to_escaped_array
  output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
  fail_on_bad_exit $?
  if [[ "$CONTAINER_TYPE" == "windows" ]]; then
  ...
  # ********** For testing ***************
  COMMAND=(6 "$docker_cmd" "exec" "$HNN_CONTAINER_NAME" \
          "bash" "-c" "ls -ld \"$__hnn_out_path\" && ls -l \"$__hnn_out_path\"")
  convert_COMMAND_to_escaped_array
  MSYS_NO_PATHCONV=1 output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
  # **************************************
...
function copy_ssh_files_to_container_fail {
  ...
  local __container_authorized_keys
  local __container_known_hosts
  local __container_user

  if [[ "$CONTAINER_TYPE" == "windows" ]]; then
    stop_container_silent
    __container_authorized_keys="/c/home/hnn_user/.ssh/authorized_keys"
    __container_known_hosts="/c/home/hnn_user/.ssh/known_hosts"
    __container_user="ContainerAdministrator"
  else
    __container_authorized_keys="/home/hnn_user/.ssh/authorized_keys"
    __container_known_hosts="/home/hnn_user/.ssh/known_hosts"
    __container_user="root"
  fi

  print_header_message_short "Copying authorized_keys file into container... "
  COMMAND=(4 "$docker_cmd" "cp" "$SSH_AUTHKEYS" "$HNN_CONTAINER_NAME:$__container_authorized_keys")
  convert_COMMAND_to_escaped_array
  output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
  fail_on_bad_exit $?

  print_header_message_short "Copying known_hosts file into container... "
  COMMAND=(4 "$docker_cmd" "cp" "$SSH_PUBKEY" "$HNN_CONTAINER_NAME:$__container_known_hosts")
  convert_COMMAND_to_escaped_array
  output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
  fail_on_bad_exit $?
  if [[ "$CONTAINER_TYPE" == "windows" ]]; then
  ...
  COMMAND=(8 "$docker_cmd" "exec" "-u" "$__container_user" "$HNN_CONTAINER_NAME" "bash" "-c" "chown -R hnn_user /home/hnn_user/.ssh")
  convert_COMMAND_to_escaped_array
  output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
  fail_on_bad_exit $?

  print_header_message_short "Updating permissions on ssh authorized_keys file in container... "
  COMMAND=(8 "$docker_cmd" "exec" "-u" "$__container_user" "$HNN_CONTAINER_NAME" "bash" "-c" "chmod 600 /home/hnn_user/.ssh/authorized_keys")
  convert_COMMAND_to_escaped_array
  output_run_command_arguments "${ESCAPED_COMMAND[@]}" &> /dev/null
  fail_on_bad_exit $?
}
...
function docker_run_fail {
  ...
  if [[ "$CONTAINER_TYPE" == "windows" ]]; then
    __container_xauthority="/c/windows/temp/.Xauthority"
  fi
...
function get_xauth_keys {
  ...
  # to log file
  echo -e "\n  ** Command: \"${xauth_cmd}\" -f \"$XAUTHORITY\" nlist $DISPLAY" >> "$LOGFILE"
  echo -n "  ** Stderr: " >> "$LOGFILE"
...
function get_container_xauth_key {
  ...
  __keys=$(SUPPRESS_OUTPUT=1 output_run_command_arguments "${ESCAPED_COMMAND[@]}")
  if [[ $? -ne 0 ]]; then
    exit 2
  else
    echo "$(echo $__keys | grep '^ffff' | head -1 | awk '{print $NF}')"
  fi
}
...
function get_docker_container_type {
  ...
  if [[ ! -z $__output ]] && [[ $? -ne "0" ]]; then
    ...
    if [[ "$CONTAINER_TYPE" == "windows" ]]; then
      echo -e "**Windows containers are not supported yet**\n" | tee -a "$LOGFILE"
    fi
  fi
